...
export const investigateOrderSchema = z.object({
  order_id: z
    .string()
    .describe("REQUIRED. The order's MongoDB _id (24-char hex string). Investigates what happened and why."),
});
...
export async function investigateOrder(params: InvestigateOrderInput) {
  ...
  const [driverDoc, restaurantDoc, cityDoc] = await Promise.all([
    raw.driver_id ? Driver.findById(raw.driver_id, { username: 1, last_name: 1, currentStatus: 1, last_update_time: 1 }).lean() : null,
    raw.restaurant_id ? Restaurant.findById(raw.restaurant_id, { restaurantname: 1, status: 1, restaurantAvailability: 1 }).lean() : null,
    raw.main_city ? City.findOne(
      { $or: [{ state: raw.main_city }, { cityname: raw.main_city }], country_code: raw.country_code },
      { max_dispatch_time: 1, timer_config: 1, dispatch_delay_time: 1, state: 1 }
    ).lean() : null,
  ]);
  ...
  addEvent("Driver accepted", history.driver_accepted,
    driver ? `Driver: ${[driver.username, driver.last_name].filter(Boolean).join(" ")}` : undefined);
  ...
  const result = {
    ...
    timeline,
    driver: driver ? { id: String(driver._id), name: [driver.username, driver.last_name].filter(Boolean).join(" ") || null } : null,
    restaurant: restaurant ? { id: String((restaurant as Record<string, unknown>)._id), name: (restaurant as Record<string, unknown>).restaurantname } : null,
