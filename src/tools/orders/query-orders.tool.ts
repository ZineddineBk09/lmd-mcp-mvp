...
export const queryOrdersSchema = z.object({
  ...
  city: z.string().optional().describe('OPTIONAL. City name (main_city). Omit to query the entire country.'),
  status: z
    .preprocess((val) => {
  ...
  since_minutes: z.number().optional().describe('OPTIONAL. Only orders created in the last N minutes. Omit for no time filter.'),
  restaurant_id: z.string().optional().describe('OPTIONAL. Filter by specific restaurant ObjectId.'),
...
export async function queryOrders(params: QueryOrdersInput) {
  ...
  for (const order of orders) {
    const label = ORDER_STATUS_LABELS[order.status] || `Unknown(${order.status})`;
