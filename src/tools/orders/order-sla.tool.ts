...
export async function getOrderSlaStatus(params: OrderSlaInput) {
  ...
  const cityConfigs = await City.find(params.country_code ? { country_code: params.country_code } : {}, { timer_config: 1, cityname: 1, state: 1 }).lean();

  const timerEnabled = cityConfigs.some((c) => c.timer_config?.isEnabled);
  const slaMinutes = timerEnabled ? Math.max(...cityConfigs.map((c) => c.timer_config?.storeTimer || DEFAULT_SLA_MINUTES)) : DEFAULT_SLA_MINUTES;
  ...
  const pipeline = [
    ...
    },
    {
      $addFields: {
        sla_status: {
          $cond: [{ $gte: ['$sla_pct', 100] }, 'BREACHED', { $cond: [{ $gte: ['$sla_pct', 80] }, 'AT_RISK', 'HEALTHY'] }],
        },
      },
    },
    {
  ...
  return wrapToolResponse(
    {
      ...
      health_pct: totalActive > 0 ? Math.round(((totalActive - breached.length) / totalActive) * 100) : 100,
