...
export async function getEtaAccuracy(params: EtaAccuracyInput) {
  const cacheKey = buildCacheKey('eta_accuracy', params as Record<string, unknown>);
  ...
  const overallPipeline: Record<string, unknown>[] = [
    ...
    addFieldsStage,
    {
      $group: {
        ...
        on_time_count: {
          $sum: {
            $cond: [{ $lte: ['$actual_delivery_minutes', ON_TIME_THRESHOLD_MINUTES] }, 1, 0],
          },
        },
  ...
  const [overallResult, byCityResult] = await Promise.all([Order.aggregate(overallPipeline as never[]), Order.aggregate(byCityPipeline as never[])]);
  ...
  const byCity = byCityResult.map((r) => ({
    ...
    on_time_rate_pct: (r.total_delivered ?? 0) > 0 ? Math.round(((r.on_time_count ?? 0) / (r.total_delivered ?? 1)) * 100) : 0,
  }));
  ...
  const response = wrapToolResponse(
    {
      ...
      overall: {
        ...
        avg_actual_minutes: Math.round((overall?.avg_actual_minutes ?? 0) * 10) / 10,
        min_actual_minutes: overall?.min_actual_minutes != null ? Math.round((overall.min_actual_minutes as number) * 10) / 10 : null,
        max_actual_minutes: overall?.max_actual_minutes != null ? Math.round((overall.max_actual_minutes as number) * 10) / 10 : null,
      },
      by_city: byCity,
      summary,
    },
