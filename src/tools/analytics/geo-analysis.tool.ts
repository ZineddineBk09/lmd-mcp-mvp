...
export const geoAnalysisSchema = z.object({
  ...
  analysis: z.enum(['driver_density', 'unassigned_hotspots']).default('driver_density').describe('OPTIONAL. Type of analysis.'),
});
...
export async function getGeoAnalysis(params: GeoAnalysisInput) {
  ...
  if (params.analysis === 'driver_density') {
    ...
    const zones = results.map((r) => ({
      ...
      busy: r.busy,
      utilization_pct: r.total_drivers > 0 ? Math.round((r.busy / r.total_drivers) * 100) : 0,
    }));
  ...
  const hotspots = results.map((r) => ({
    ...
    oldest_order_minutes_ago: r.oldest_order ? Math.round((now - new Date(r.oldest_order).getTime()) / 60000) : null,
  }));
