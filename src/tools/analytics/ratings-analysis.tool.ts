...
export const ratingsAnalysisSchema = z.object({
  ...
  min_rating: z
    .number()
    .default(1)
    .describe("OPTIONAL. Minimum rating to include (default 1)."),
  max_rating: z
    .number()
    .default(3)
    .describe(
      "OPTIONAL. Maximum rating to include (default 3, for complaints).",
    ),
  since_hours: z
    .number()
    .default(168)
    .describe("OPTIONAL. Time window in hours (default 168 = 7 days)."),
  group_by: z
    .enum(["restaurant", "driver", "rating"])
    .default("restaurant")
    .describe(
      "OPTIONAL. Group results by restaurant, driver, or rating value.",
    ),
...
export async function getRatingsAnalysis(params: RatingsAnalysisInput) {
  ...
  const groupField =
    params.group_by === "restaurant"
      ? "$rating_to"
      : params.group_by === "driver"
        ? "$driver_id"
        : "$rating";
        ...
        .find(
          {
            _id: {
              $in: ids.map(
                (id: unknown) => new mongoose.Types.ObjectId(String(id)),
              ),
            },
          },
  ...
  const rows = enriched.map((r) => ({
    ...
    sample_comments: (r.sample_comments ?? []).filter(
      (c: string) => c.length > 0,
    ),
  }));
