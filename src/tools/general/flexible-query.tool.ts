...
export const flexibleQuerySchema = z.object({
  collection: z.string().describe('REQUIRED. MongoDB collection name. Examples: orders, restaurant, drivers, city, countrycurrency, cartv2, food, users, offer, etc.'),
  action: z.enum(['count', 'find', 'distinct']).describe("REQUIRED. 'count' returns document count, 'find' returns documents, 'distinct' returns unique values."),
  filter: z
    .record(z.unknown())
    .describe(
  ...
  distinct_field: z.string().optional().describe("OPTIONAL. Field for 'distinct' action."),
  projection: z.record(z.number()).optional().describe('OPTIONAL. Fields to include (1) or exclude (0).'),
  sort: z.record(z.number()).optional().describe('OPTIONAL. Sort order. Example: {createdAt:-1}'),
  limit: z.number().default(25).describe('OPTIONAL. Max documents, capped at 50 (default 25).'),
});

export type FlexibleQueryInput = z.infer<typeof flexibleQuerySchema>;

function validateFilter(filter: Record<string, unknown>, depth = 0): string | null {
  if (depth > MAX_FILTER_DEPTH) return 'Filter too deeply nested (max 3 levels).';
  if (Object.keys(filter).length === 0) return 'Filter cannot be empty — include at least one condition.';
  ...
  for (const [key, value] of Object.entries(filter)) {
    ...
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      const nested = validateFilter(value as Record<string, unknown>, depth + 1);
...
function sanitizeProjection(projection?: Record<string, number>): Record<string, number> | undefined {
...
function isBsonLeaf(value: unknown): boolean {
  ...
  if (value && typeof value === 'object') {
    const constructor = (value as Record<string, unknown>).constructor;
    if (constructor && ['Binary', 'Decimal128', 'Long', 'Timestamp', 'Double', 'Int32'].includes(constructor.name)) return true;
  }
  return false;
}
...
function extractFieldPaths(obj: Record<string, unknown>, prefix = '', depth = 0): string[] {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value && typeof value === 'object' && !Array.isArray(value) && !isBsonLeaf(value) && Object.keys(value).length > 0) {
      paths.push(...extractFieldPaths(value as Record<string, unknown>, fullPath, depth + 1));
    }
  }
  return paths;
}
...
function findBestFieldMatch(filterKey: string, actualPaths: string[]): string | null {
...
function autoCorrectFilter(filter: Record<string, unknown>, actualPaths: string[]): { corrected: Record<string, unknown>; corrections: string[] } {
...
export async function flexibleQuery(params: FlexibleQueryInput) {
  ...
  if (BLOCKED_COLLECTIONS.has(params.collection)) {
    return wrapToolResponse({ error: `Collection '${params.collection}' is not accessible.` }, { query: 'BLOCKED', execution_time_ms: 0, result_count: 0 });
  }

  if (DEPRECATED_COLLECTIONS.has(params.collection)) {
    const redirect = COLLECTION_REDIRECTS[params.collection];
    const hint = redirect ? `Use '${redirect}' instead.` : `This collection is empty or deprecated.`;
  ...
  if (filterError) {
    return wrapToolResponse({ error: filterError }, { query: 'BLOCKED', execution_time_ms: 0, result_count: 0 });
  }
  ...
  try {
    ...
    switch (params.action) {
      ...
      case 'find': {
        ...
        resultCount = total;
        const docs = rawDocs.map((d) => stripBlockedFields(d as Record<string, unknown>, params.collection));
      ...
      case 'distinct': {
        if (!params.distinct_field) {
          return wrapToolResponse({ error: "distinct_field is required when action is 'distinct'." }, { query: 'BLOCKED', execution_time_ms: 0, result_count: 0 });
        }
        const dfLower = params.distinct_field!.toLowerCase();
        const collKeys = COLLECTION_REDACTED_KEYS[params.collection] ?? [];
        if (BLOCKED_FIELDS.some((bf) => dfLower.includes(bf)) || collKeys.includes(params.distinct_field!)) {
          return wrapToolResponse({ error: `Field '${params.distinct_field}' is blocked.` }, { query: 'BLOCKED', execution_time_ms: 0, result_count: 0 });
        }
    ...
    return wrapToolResponse(
      {
        ...
        ...(actualFields.length > 0 ? { available_fields: actualFields.slice(0, 40) } : {}),
      },
      {
