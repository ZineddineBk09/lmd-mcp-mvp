...
export const listCollectionsSchema = z.object({
  include_empty: z.boolean().default(false).describe('OPTIONAL. If true, include collections with 0 documents (default: false).'),
});
...
export async function listCollections(params: ListCollectionsInput) {
  const cacheKey = buildCacheKey('list_collections', params as Record<string, unknown>);
    ...
    const response = wrapToolResponse(
      {
        ...
        collections: summary,
        usage_hint: 'Pick a collection name, then call describe_collection to see its fields before querying with flexible_query.',
      },
      {
