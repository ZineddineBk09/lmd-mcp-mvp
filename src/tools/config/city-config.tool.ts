...
export const cityConfigSchema = z.object({
  country_code: z.string().optional().describe('OPTIONAL. Country code: DZ, MA, TN, FR, SN, ZA, etc. Omit to see all countries.'),
  city: z.string().optional().describe('OPTIONAL. City name (exact or partial match).'),
  field: z.string().optional().describe("OPTIONAL. Specific config field to focus on (e.g. 'auto_dispatch', 'max_orders', 'driver_radius', 'dispatch_delay_time', 'timer_config')."),
});
...
export async function cityConfig(params: CityConfigInput) {
  const cacheKey = buildCacheKey('city_config', params as Record<string, unknown>);
  ...
  if (params.field) {
    ...
    result = {
      ...
      summary: `${params.field} across ${comparison.length} cities: ${uniqueValues.length} distinct values. ${comparison
        .slice(0, 3)
        .map((c: Record<string, unknown>) => `${c.city}=${JSON.stringify(c[params.field!])}`)
