...
export const ghostDriversSchema = z.object({
  ...
  stale_threshold_minutes: z.number().default(5).describe('OPTIONAL. Inactivity threshold in minutes (default 5).'),
...
export async function getGhostDrivers(params: GhostDriversInput) {
  ...
  const staleThreshold = Date.now() - params.stale_threshold_minutes * 60 * 1000;
