...
export async function fleetStatus(params: FleetStatusInput) {
  const cacheKey = buildCacheKey('fleet_status', params as Record<string, unknown>);
  ...
  const [onlineCount, busyCount, ghostCount, offlineCount, totalCount] = await Promise.all([
  ...
  const supplyHealth = availableCount === 0 ? 'CRITICAL' : availableCount < 5 ? 'LOW' : availableCount < 15 ? 'MODERATE' : 'HEALTHY';
