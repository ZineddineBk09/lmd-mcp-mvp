...
export async function getSupplyDemandBalance(params: SupplyDemandInput) {
  const cacheKey = buildCacheKey('supply_demand_balance', params as Record<string, unknown>);
  ...
  const ratio = availableCount > 0 ? Math.round((activeOrders / availableCount) * 10) / 10 : activeOrders > 0 ? Infinity : 0;
  ...
  if (severity === 'CRITICAL' || severity === 'HIGH') {
    recommendations.push(`Widen dispatch radius from ${cityConfig?.driver_radius || 20} to ${(cityConfig?.driver_radius || 20) + 10}`);
