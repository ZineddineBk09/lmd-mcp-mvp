...
export const lookupRestaurantSchema = z.object({
  restaurant_id: z
    .string()
    .optional()
    .describe("MongoDB _id of the restaurant. Provide exactly one of restaurant_id or name."),
  name: z
    .string()
    .optional()
    .describe("Restaurant name (partial match, case-insensitive). Provide exactly one of restaurant_id or name."),
});
...
export async function lookupRestaurant(params: LookupRestaurantInput) {
  ...
  if (restaurants.length === 0) {
    ...
    return wrapToolResponse(
      ...
      { query: `db.restaurant.find(${JSON.stringify(filter)})`, execution_time_ms: Date.now() - start, result_count: 0 }
    );
  }
  ...
  const results = await Promise.all(
    restaurants.map(async (doc) => {
      ...
      const [activeOrders, todayStats, menuCount] = await Promise.all([
        Order.find(
          { restaurant_id: restId, status: { $in: ACTIVE_ORDER_STATUSES } },
        ...
        Order.aggregate([
          { $match: { restaurant_id: restId, createdAt: { $gte: todayStart } } },
          {
            $group: {
              _id: null,
              total: { $sum: 1 },
              delivered: { $sum: { $cond: [{ $eq: ["$status", 7] }, 1, 0] } },
              rejected_by_restaurant: { $sum: { $cond: [{ $eq: ["$status", 2] }, 1, 0] } },
        ...
        mongoose.connection.db
          ? mongoose.connection.db.collection("food").countDocuments({ restaurant_id: restId })
          : 0,
      ]);

      const stats = todayStats[0] ?? { total: 0, delivered: 0, rejected_by_restaurant: 0, cancelled: 0, timed_out: 0 };
      const acceptRate =
        stats.total > 0
          ? Math.round(((stats.total - stats.rejected_by_restaurant) / stats.total) * 100)
          : null;
      ...
      return {
        ...
        active_orders: activeOrders.slice(0, 5).map((o: Record<string, unknown>) => ({
          _id: String(o._id),
          status: o.status,
  ...
  return wrapToolResponse(
    results.length === 1 ? results[0] : { restaurants: results, count: results.length },
    {
      query: `db.restaurant.find(...) + orders aggregation + food count`,
